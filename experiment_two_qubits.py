...
def two_qubits_compare(gamma: float=0.1,
		   phi:float = 2*np.pi,
		   L: float = 2, 
		   c:float = 1,
		   t_max:float=40,
...
def exp_two_qubits(gamma: float=0.1,
	...
	for i,phi in enumerate(phi_list):
		Delta = phi/np.pi 
		data_dde,data_ww = two_qubits_compare(gamma=gamma,phi=phi,L=L,c=c,t_max=t_max,n_steps=n_steps,plot_bool=False)
