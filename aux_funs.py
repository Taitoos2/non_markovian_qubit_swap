...
def dde_scalar_simple(
    t_max: float,
    gamma1,  # float | Callable[[float], float]
    gamma2,  # float | Callable[[float], float]
    phi: float = 0.0,  # kept for interface compatibility (unused here)
    tau: float = 1.0,
    N: int = 2,
    dt_max: float = 1e-2,
    ...
    if N != 2:
        raise ValueError("This equation is defined for N=2 (c1, c2).")
    ...
    c_list[0] = np.array([1.0 + 0.0j, 0.0 + 0.0j], dtype=np.complex128)

    zero_current = np.array([0.0 + 0.0j, 0.0 + 0.0j], dtype=np.complex128)

    def zero_current_interpolator(t: np.floating) -> np.ndarray:
        return zero_current  # (N,)
    ...
    def derivative(t, y):
        c = y.reshape(shape)  # (N,) = (2,)
        c1, c2 = c[0], c[1]

        # delayed state (N,) and delayed gamma1
        c_past = interpolator(t - tau)  # (N,)
        c1_past = c_past[0]

        gamma_now = gamma_of_t(t)  # [g1(t), g2(t)]
        gamma_past = gamma_of_t(t - tau)  # [g1(t-tau), g2(t-tau)]
        g1_now, g2_now = float(gamma_now[0]), float(gamma_now[1])
        g1_past = float(gamma_past[0])

        dc1 = -0.5 * g1_now * c1
        dc2 = -0.5 * g2_now * c2 - np.sqrt(g2_now * g1_past) * c1_past

        return np.array([dc1, dc2], dtype=np.complex128).reshape(-1)
    ...
    # integrate on [t_start, t_start + t_max]
    t0 = float(t_start)
    t_end = t0 + float(t_max)

    # store the initial point at t_start
    add_solution(np.float64(t_start), c_list[0])
...
def dde_scalar(
    ...
    def derivative(t, y):
        c = y.reshape(shape)
        a_out_past = interpolator(t - tau)

        a_in = phase * a_out_past[0, right] + phase * a_out_past[1, left]
    ...
    # --- interpret t_max as duration; integrate on [t_start, t_start + t_max]
    t0 = float(t_start)
    t_end = t0 + float(t_max)
...
def paralelizar(parameter_list, f, ncores: int = 70):
    resultados = Parallel(n_jobs=ncores, backend="loky")(
        delayed(f)(param) for param in parameter_list
    )
    return resultados
...
def fast_fft_zp(t: np.ndarray, y: np.ndarray, pad_factor=8):
    """
    FFT con zero padding.
    pad_factor: cuántas veces más larga la FFT (8 = 8N puntos)
    """
